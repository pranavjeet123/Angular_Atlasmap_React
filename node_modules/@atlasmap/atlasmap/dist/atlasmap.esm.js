import { ConfigModel, CommonUtil, ExpressionModel, ErrorInfo, ErrorLevel, ErrorScope, ErrorType, InitializationService, Multiplicity, Field, TransitionMode, TransitionModel, FieldAction, FieldType, DocumentType, DocumentInitializationModel, MappingSerializer, constantTypes, collectionTypes, getCsvParameterOptions, propertyTypes } from '@atlasmap/core';
import ky from 'ky';
import React, { useState, useEffect, useCallback, useRef, useContext, useMemo, createContext, useLayoutEffect, forwardRef, Children, isValidElement, cloneElement, memo, useReducer } from 'react';
import { scaleLinear } from 'd3-scale';
import { useGesture } from 'react-use-gesture';
import { css } from '@patternfly/react-styles';
import { linkHorizontal, linkVertical } from 'd3-shape';
import { Title, InputGroup, TextInput, Button, Label, FormSelect, FormSelectOptionGroup, FormSelectOption, Dropdown, Select, SelectOption, Form, FormGroup, Tooltip, ToolbarGroup, ToolbarItem, Modal, ValidatedOptions, InputGroupText, Checkbox, Switch, Card, CardHeader, CardActions, CardTitle, CardBody, DropdownToggle, DropdownToggleAction, DropdownItem, DropdownSeparator, CardFooter, EmptyState, EmptyStateVariant, EmptyStateIcon, Spinner, Split, SplitItem, SelectVariant, Alert, AlertActionCloseButton, Accordion, AccordionItem, AccordionToggle, AccordionContent, Toolbar, ToolbarContent, Stack, StackItem, Badge, Bullseye, Level, LevelItem, TextContent, Radio, AlertGroup } from '@patternfly/react-core';
import { SearchIcon, TrashIcon, PlusIcon, AngleDownIcon, AngleRightIcon, FolderOpenIcon, FolderCloseIcon, InfoAltIcon, BoltIcon, CaretDownIcon, CaretRightIcon, AddCircleOIcon, CloseIcon, EditIcon, ClipboardIcon, ExchangeAltIcon, ImportIcon, LayerGroupIcon, BullseyeIcon, GripVerticalIcon, CircleIcon, AtIcon, UnlinkIcon, LinkIcon, ProjectDiagramIcon, TableIcon, ExportIcon, ColumnsIcon, CodeIcon, EyeIcon, InfoIcon, MapMarkedIcon, MapIcon, HelpIcon } from '@patternfly/react-icons';
import { DndProvider, useDragLayer, useDrag, useDrop } from 'react-dnd';
import { HTML5Backend, getEmptyImage } from 'react-dnd-html5-backend';
import { TouchBackend } from 'react-dnd-touch-backend';
import styles$B from '@patternfly/react-styles/css/components/FormControl/form-control';
...
function getDocCSVParams(docId, isSource) {
  var doc = getDocDef(docId, ConfigModel.getConfig(), isSource);
...
function copyToClipboard(text) {
  ...
  try {
    ...
    ConfigModel.getConfig().errorService.addError(new ErrorInfo({
...
var initializationService = /*#__PURE__*/new InitializationService(api);
    ...
    transformations: mappedField.actions.map(function (a) {
      return {
        ...
        arguments: a.definition.arguments.map(function (av, idx) {
          ...
          return {
            label: ((_av$serviceObject = av.serviceObject) === null || _av$serviceObject === void 0 ? void 0 : (_av$serviceObject$tit = _av$serviceObject.title) === null || _av$serviceObject$tit === void 0 ? void 0 : _av$serviceObject$tit.length) > 0 ? av.serviceObject.title : CommonUtil.toDisplayable(av.name),
            type: av.type,
            name: av.name,
...
function errorInfoToNotification(e) {
  ...
  return {
    ...
    title: e.mapping && e.type !== ErrorType.PREVIEW ? errorTypeToString(e.type) + ": \"" + e.mapping.transition.getPrettyName() + "\"" : errorTypeToString(e.type),
...
function mappingExpressionInit() {
  ...
  if (!mapping.transition.expression) {
    mapping.transition.expression = new ExpressionModel(mapping, initializationService.cfg);
...
function getMappingActions(isSource) {
  return initializationService.cfg.fieldActionService.getActionsAppliesToField(initializationService.cfg.mappings.activeMapping, isSource, Multiplicity.ONE_TO_ONE);
}
function getMultiplicityActions(mapping) {
  if (mapping.transition.mode === TransitionMode.ONE_TO_MANY) {
    return initializationService.cfg.fieldActionService.getActionsAppliesToField(mapping, true, Multiplicity.ONE_TO_MANY);
  } else if (mapping.transition.mode === TransitionMode.MANY_TO_ONE) {
    return initializationService.cfg.fieldActionService.getActionsAppliesToField(mapping, true, Multiplicity.MANY_TO_ONE);
  } else {
    return [];
  }
}
...
function documentExists(file, isSource) {
  var docDef = getDocDefByName(file.name, ConfigModel.getConfig(), isSource);
...
var CanvasProvider = function CanvasProvider(_ref) {
  ...
  var rects = useRef([]);
  var removeRect = useCallback(function (id) {
    rects.current = rects.current.filter(function (r) {
      return r.id !== id;
    });
  }, []);
  var addRect = useCallback(function (rect) {
    removeRect(rect.id);
    rects.current = [].concat(rects.current, [rect]);
  }, [removeRect]);
  var getRects = useCallback(function () {
    return rects.current;
  }, []);
  var redrawCallbacks = useRef([]);
  var addRedrawListener = useCallback(function (cb) {
    redrawCallbacks.current = [].concat(redrawCallbacks.current, [cb]);
  }, []);
  var removeRedrawListener = useCallback(function (cb) {
  ...
  var updateZoom = useCallback(function (tick) {
  ...
  var resetPan = useCallback(function () {
    return setPan({
      x: 0,
      y: 0
    });
...
function useCanvas() {
  var context = useContext(CanvasContext);
  ...
  var _context$dimensions = context.dimensions,
      ...
      zoom = context.zoom;
  var xDomain = useMemo(function () {
    return scaleLinear().range([0, width]).domain([0, width * zoom]);
  }, [width, zoom]);
  var yDomain = useMemo(function () {
    return scaleLinear().range([height, 0]).domain([height * zoom, 0]);
  }, [height, zoom]);
...
function useDimensions(_temp) {
  ...
  var previousDimensions = useRef(noSize);

  var _useState = useState(noSize),
...
var HTMLObject = /*#__PURE__*/forwardRef(function HTMLObject(_ref, ref) {
  ...
  return React.createElement("foreignObject", Object.assign({
    width: width,
    height: height,
    x: x,
    y: y,
    className: css(styles$1.foreignObject, className)
  }, props, {
    ref: ref
  }), children);
});
...
var Arc = function Arc(_ref) {
  ...
  var link = useMemo(function () {
    return (type === 'horizontal' ? linkHorizontal : linkVertical)().context(null).source(function (d) {
      return d.start;
    }).target(function (d) {
      return d.end;
    }).x(function (d) {
  ...
  return d ? React.createElement("g", {
    ...
    strokeWidth: 20,
    fill: 'none',
    className: css(props.onClick && styles$2.clickable)
  }, props)), children) : null;
};
...
function useBoundingCanvasRect() {
  ...
  var convertClientX = useCallback(function (x) {
    return xDomain(x - offsetLeft - panX);
  }, [offsetLeft, panX, xDomain]);
  var convertClientY = useCallback(function (y) {
    return yDomain(y - offsetTop - panY);
  }, [offsetTop, panY, yDomain]);
  var convertDOMRectToCanvasRect = useCallback(function (rect) {
    return {
  ...
  return {
    getBoundingCanvasRect: useCallback(function (element) {
...
var NodeRefContext = /*#__PURE__*/createContext(null);
var NodeRefProvider = function NodeRefProvider(_ref) {
  ...
  return React.createElement(NodeRefContext.Provider, {
    value: {
      nodes: nodes,
      setRef: setRef,
      unsetRef: unsetRef
...
function useNodeRef(props) {
  var context = useContext(NodeRefContext);
  useEffect(function () {
    if (context && props.id) {
...
function useNodeRect() {
  ...
  var getRect = useCallback(function (id) {
    if (!context) {
      return null;
    }
...
var NodeRef = /*#__PURE__*/forwardRef(function NodeRef(_ref, ref) {
  var children = _ref.children,
      props = _objectWithoutPropertiesLoose(_ref, ["children"]);

  var node = Children.only(children);
...
var NodesArc = function NodesArc(_ref) {
  ...
  var _useState2 = useState('horizontal'),
      linkType = _useState2[0],
      setLinkType = _useState2[1];

  var calculateCoords = useCallback(function calculateCoordsCb() {
  ...
  useEffect(function onNodeArcRedrawCb() {
  ...
  return coords ? React.createElement(Arc, Object.assign({
    start: coords.start,
    end: coords.end,
    type: linkType,
...
var Column = function Column(_ref) {
  ...
  return React.createElement("div", Object.assign({
    className: css(styles$3.column, !visible && styles$3.hidden),
    style: style
  }, props), children);
};
...
/*
    ...
    limitations under the License.
*/
var ColumnBody = /*#__PURE__*/forwardRef(function ColumnBody(_ref, ref) {
  var children = _ref.children;
  return React.createElement("div", {
...
var Actions = function Actions(_ref) {
  ...
  }, Children.map(children, function (c, idx) {
    return React.createElement("div", {
...
var ColumnHeader = function ColumnHeader(_ref) {
  ...
  return React.createElement("div", {
    ...
    className: css(styles$6.toolbar, variant === 'plain' && styles$6.plain)
  }, React.createElement("div", {
    className: styles$6.title
  }, React.createElement(Title, {
    headingLevel: "h2",
    size: "lg"
  }, title)), React.createElement(Actions, null, actions === null || actions === void 0 ? void 0 : actions.filter(function (a) {
    return a;
  }))), children);
};
...
var ColumnMapper = function ColumnMapper(_ref) {
  ...
  return React.createElement(Canvas, Object.assign({}, props, {
    className: css(styles$7.canvas, className)
  }), children);
};
...
var SearchableColumnHeader = function SearchableColumnHeader(_ref) {
  ...
  return React.createElement(ColumnHeader, {
    ...
    actions: [React.createElement("form", {
      ...
      variant: 'control'
    }, React.createElement(SearchIcon, null))))].concat(actions)
  });
};
...
var FieldsDndContext = /*#__PURE__*/createContext(null);
var FieldsDndProvider = function FieldsDndProvider(_ref3) {
  var children = _ref3.children;
  var hoveredTarget = useRef(null);
  ...
  return React.createElement(Provider, null, React.createElement(FieldsDndContext.Provider, {
    value: {
...
function useFieldsDnd() {
  var context = useContext(FieldsDndContext);
...
var DraggableField = /*#__PURE__*/forwardRef(function DraggableField(_ref, ref) {
...
var FieldDropTarget = /*#__PURE__*/forwardRef(function FieldDropTarget(_ref, ref) {
  ...
  var wasOver = useRef(false);
  var wasTarget = useRef(false);
  useEffect(function () {
...
var FieldDragLayer = function FieldDragLayer() {
  ...
  useEffect(measure);
  return React.createElement(DraggedField, null, function (_ref) {
    var isDragging = _ref.isDragging,
        currentOffset = _ref.currentOffset,
        draggedField = _ref.draggedField;
    return isDragging && currentOffset && draggedField ? React.createElement("div", {
      style: {
        position: 'absolute',
        zIndex: 1000,
        width: dimensions.width,
    ...
    }, React.createElement(Label, null, draggedField.name))) : null;
  });
};
...
var AddFieldTypeahead = function AddFieldTypeahead(_ref) {
  ...
  var renderOptions = function renderOptions(fields, placeholderField) {
    ...
    var groups = fields.reduce(function (groups, f) {
      ...
      groups[f.group] = [].concat(groups[f.group] || [], [React.createElement(FormSelectOption, {
  ...
  var onChange = useCallback(function (value, _e) {
  ...
  return React.createElement("div", null, React.createElement(FormSelect, {
...
var AutoDropdown = function AutoDropdown(_ref) {
  ...
  return React.createElement(Dropdown, Object.assign({}, props, {
    isOpen: isOpen,
    onSelect: toggleOpen,
    toggle: toggle({
      isOpen: isOpen,
...
var ExpressionContent = function ExpressionContent(_ref) {
  ...
  var initMappingExpression = useCallback(function () {
    initializeMappingExpression();
  }, []);
  ...
  return React.createElement(React.Fragment, null, React.createElement(Form, null, React.createElement(FormGroup, {
    fieldId: "expressionContent"
  }, React.createElement(InputGroup, null, React.createElement(Tooltip, {
    ...
    isDisabled: disabled
  }, React.createElement("i", null, "f", React.createElement("small", {
    style: {
      position: 'relative',
      bottom: -3
    }
  }, "(x)")))), !disabled && mappingExpression !== undefined ? React.createElement(Tooltip, {
    ...
    className: css(styles$B.formControl, 'ExpressionFieldSearch'),
  ...
  ))), React.createElement("div", null, mappedFieldCandidates.length > 0 && React.createElement("span", null, React.createElement(ExpressionFieldSearch, {
    clearSearchMode: clearSearchMode,
    insertSelectedField: insertSelectedField,
    mappedFieldCandidates: mappedFieldCandidates
  })), showEnumSelect && React.createElement("span", null, React.createElement(ExpressionEnumSelect, {
...
var ConditionalExpressionInput = function ConditionalExpressionInput(_ref) {
  ...
  return React.createElement(ToolbarGroup, {
    ...
    className: styles$d.toolbarItem
  }, React.createElement(ExpressionContent, {
    disabled: disabled,
...
var AboutDialog = function AboutDialog(_ref) {
  ...
  }, React.createElement("div", null, "UI Version: ", uiVersion), React.createElement("div", null, "Runtime Version: ", runtimeVersion));
};
...
var ConstantDialog = function ConstantDialog(_ref) {
  ...
  var _useState3 = useState(initialValueType),
  ...
  var _useState6 = useState(ValidatedOptions["default"]),
  ...
  return React.createElement(ConfirmationDialog, {
    ...
    fieldId: 'name',
    isRequired: true
  }, !isConstantNameUnique ? React.createElement(Tooltip, {
    content: React.createElement("div", null, "A constant with this name already exists"),
    entryDelay: 750,
    exitDelay: 100
  }, React.createElement(TextInput, {
    value: name,
...
var CustomClassDialog = function CustomClassDialog(_ref) {
  ...
  var _useState = useState(initialCustomClassName),
      customClassName = _useState[0],
      setCustomClassName = _useState[1];

  var _useState2 = useState(initialCollectionType),
...
var ExportCatalogDialog = function ExportCatalogDialog(_ref) {
  ...
  var _useState = useState(defaultCatalogName),
  ...
  }), React.createElement(InputGroupText, null, ".adm")))));
};
...
var LookupTableDialog = function LookupTableDialog(_ref) {
  ...
  var _useState = useState(enumerationValue.selectedTargetEnumValue),
  ...
  }, enumerationValue.sourceEnumValue), React.createElement(FormSelect, {
...
var NamespaceDialog = function NamespaceDialog(_ref) {
  ...
  var _useState = useState(initialAlias),
      alias = _useState[0],
      setAlias = _useState[1];

  var _useState2 = useState(initialUri),
      uri = _useState2[0],
      setUri = _useState2[1];

  var _useState3 = useState(initialLocationUri),
      locationUri = _useState3[0],
      setLocationUri = _useState3[1];

  var _useState4 = useState(initialTargetNamespace),
...
var PropertyDialog = function PropertyDialog(_ref) {
  ...
  var _useState2 = useState(initialValueType),
      valueType = _useState2[0],
      setValueType = _useState2[1];

  var _useState3 = useState(initialScope),
      scope = _useState3[0],
  ...
  var _useState5 = useState(ValidatedOptions["default"]),
  ...
  return React.createElement(ConfirmationDialog, {
    ...
    fieldId: 'name',
    isRequired: true
  }, !isNameAndScopeUnique ? React.createElement(Tooltip, {
    content: React.createElement("div", null, "A property with this name and scope already exists"),
    entryDelay: 750,
    exitDelay: 100
  }, React.createElement(TextInput, {
    value: name,
...
var ParametersDialog = function ParametersDialog(_ref) {
  ...
  var handleChangeParameter = useCallback(function (index, availableParameterIndex) {
  ...
  return React.createElement(ConfirmationDialog, {
    ...
    return React.createElement(FormGroup, {
      ...
      label: parameter.required ? React.createElement("span", null, parameter.label) : React.createElement(FormSelect, {
    ...
    }, React.createElement(InputGroup, null, parameter["boolean"] ? React.createElement("span", {
      style: {
        paddingTop: 5,
        display: 'inline',
        marginLeft: 'auto',
    ...
    })) : parameter.options && parameter.options.length > 0 ? React.createElement(FormSelect, {
    ...
    }), !parameter.required && React.createElement(Button, {
      variant: "plain",
      "aria-label": "Action",
...
var Document = /*#__PURE__*/forwardRef(function Document(_ref, ref) {
  var title = _ref.title,
      actions = _ref.actions,
      footer = _ref.footer,
  ...
  return React.createElement("div", Object.assign({
    className: css(stacked && styles$g.stacked),
    onClick: handleClick,
    onKeyDown: handleKey,
    ref: ref
  }, props), React.createElement(Card, {
    isCompact: true,
    className: css(styles$g.card, noShadows && styles$g.noShadows, dropAccepted && !dropTarget && styles$g.dropAccepted, dropTarget && styles$g.dropTarget),
    isSelected: makeCardSelected,
    isSelectable: makeCardSelected || selectable,
    "aria-label": title
  }, (title || actions) && React.createElement(CardHeader, {
    "data-codemods": "true",
    className: css(styles$g.head)
  }, React.createElement(CardActions, {
    className: css(styles$g.actions)
  }, actions === null || actions === void 0 ? void 0 : actions.filter(function (a) {
    return a;
  })), title && React.createElement(CardTitle, {
    className: css(styles$g.header)
  }, React.createElement(Button, {
    variant: 'plain',
    onClick: toggleExpanded,
    "aria-label": 'Expand/collapse this card',
    "data-testid": "expand-collapse-" + title + "-button",
    className: css(styles$g.headerButton)
  }, React.createElement(Title, {
    size: "lg",
    headingLevel: 'h2',
    "aria-label": title
  }, React.createElement(TruncatedString, {
    title: title
  }, isExpanded ? React.createElement(AngleDownIcon, null) : React.createElement(AngleRightIcon, null), ' ', title))))), React.createElement(CardBody, {
    className: css(noPadding ? styles$g.bodyNoPadding : styles$g.bodyWithPadding, !isExpanded && styles$g.hidden)
  }, children), footer));
});
...
var DocumentActions = function DocumentActions(_ref) {
  ...
  return React.createElement(Dropdown, {
    toggle: React.createElement(DropdownToggle, {
      splitButtonItems: [React.createElement(DropdownToggleAction, {
        key: "action",
    ...
    dropdownItems: [React.createElement(DropdownItem, {
      icon: React.createElement(FolderCloseIcon, null),
      key: 'collapse',
      onClick: onCollapseFields
    }, "Collapse all"), React.createElement(DropdownSeparator, {
      key: 'sep-1'
...
/*
    ...
    limitations under the License.
*/
var DocumentField = /*#__PURE__*/forwardRef(function DocumentField(_ref, ref) {
  var name = _ref.name,
      type = _ref.type,
      scope = _ref.scope,
      icon = _ref.icon,
  ...
  return React.createElement("div", {
    ref: ref,
    className: css(styles$h.element, isDragging && styles$h.isDragging, isSelected && styles$h.isSelected, isDisabled && styles$h.isDisabled),
  ...
  }, React.createElement(TruncatedString, null, name), React.createElement("span", null, showType && " (" + type + ")"), React.createElement("span", {
    className: styles$h.statusIcons
  }, statusIcons && (statusIcons === null || statusIcons === void 0 ? void 0 : statusIcons.filter(function (a) {
    return a;
  }))))), (isHovering || isFocused) && actions && !isDisabled && React.createElement("div", {
...
/*
    ...
    limitations under the License.
*/
var DocumentGroup = /*#__PURE__*/forwardRef(function DocumentGroup(_ref, ref) {
  var name = _ref.name,
      type = _ref.type,
      icon = _ref.icon,
  ...
  }, expanded ? React.createElement(FolderOpenIcon, null) : React.createElement(FolderCloseIcon, null)), name, showType && " (" + type + ")", icon && React.createElement(Tooltip, {
    enableFlip: true,
    content: iconTooltip
  }, React.createElement("span", {
...
var MappingTransformation = function MappingTransformation(_ref) {
  ...
  var RenderTransformationArgumentOptions = function RenderTransformationArgumentOptions(argId, arg, disableTransformation, onTransformationArgumentChange) {
    ...
    var _useState2 = useState(initialSelected),
        selected = _useState2[0],
        setSelected = _useState2[1];

    var _useState3 = useState(initialUserDefined),
    ...
    return React.createElement(Split, null, React.createElement(SplitItem, null, React.createElement(Select, {
    ...
    }))), isUserDefined(selected) && React.createElement(SplitItem, null, React.createElement(TextInput, {
      id: "userDefined",
      type: "text",
      name: "userDefined",
  ...
  var renderTransformationArgumentBoolean = function renderTransformationArgumentBoolean(argId, a) {
    return React.createElement(Checkbox, {
      className: css(styles$m.transArgs),
      id: argId,
  ...
  })), onRemoveTransformation && React.createElement(Button, {
    variant: 'plain',
    onClick: onRemoveTransformation,
    "data-testid": "close-transformation-" + name + "-button",
    "aria-label": "Remove the transformation"
  }, React.createElement(TrashIcon, null)))), transformationsArguments.map(renderTransformationArgument));
};
...
var MappingField = function MappingField(_ref) {
  ...
  return React.createElement(FieldDropTarget, {
    ...
    return React.createElement(DraggableField, {
      field: {
        type: 'mapping',
        id: field.id,
        name: field.name,
      ...
      return React.createElement(NodeRef, {
      ...
      }, canShowIndex && React.createElement(Tooltip, {
        position: 'auto',
        enableFlip: true,
        entryDelay: 750,
        exitDelay: 100,
        content: React.createElement("div", null, "Edit the index for this element by selecting the arrows or by dragging and dropping the element to the desired position. Placeholders may be automatically inserted to account for any gaps in the indexing.")
      }, React.createElement(InputGroup, {
        className: styles$n.fieldIndex
      }, React.createElement(InputGroupText, null, "#"), React.createElement(TextInput, {
        type: 'number',
        value: index,
        id: 'index',
        ...
        entryDelay: 750,
        exitDelay: 100,
        content: React.createElement("div", null, info)
      }, React.createElement("div", {
        className: styles$n.fieldName
      }, name, " ", React.createElement(InfoAltIcon, null))))), !mappingExpressionEnabled && onNewTransformation && React.createElement(SplitItem, null, React.createElement(Tooltip, {
        position: 'auto',
        enableFlip: true,
        entryDelay: 750,
        exitDelay: 100,
      ...
      }, React.createElement(BoltIcon, null)))), React.createElement(SplitItem, null, React.createElement(Tooltip, {
        position: 'auto',
        enableFlip: true,
        entryDelay: 750,
        exitDelay: 100,
      ...
      }, React.createElement(TrashIcon, null))))), !mappingExpressionEnabled && hasTransformations && children && Children.count(children) > 0 && React.createElement("div", {
        className: css(styles$n.transformationsWrapper)
      }, React.createElement(Title, {
        headingLevel: "h6",
        size: "md"
      }, "Transformations"), React.createElement("div", {
...
var MappingFields = function MappingFields(_ref) {
  ...
  }, expanded ? React.createElement(CaretDownIcon, null) : React.createElement(CaretRightIcon, null)), title, React.createElement(Tooltip, {
    position: 'top',
    enableFlip: true,
    content: React.createElement("div", null, "Create and map a property."),
    key: 'create-property',
    entryDelay: 750,
    exitDelay: 100
  }, React.createElement(Button, {
  ...
  }, React.createElement(AddCircleOIcon, null))), isSource && React.createElement(Tooltip, {
    position: 'top',
    enableFlip: true,
    content: React.createElement("div", null, "Create and map a source constant."),
    key: 'create-constant',
    entryDelay: 750,
    exitDelay: 100
  }, React.createElement(Button, {
  ...
  }, React.createElement(PlusIcon, null))), expanded && children);
};
...
var TimedToast = function TimedToast(_ref) {
  ...
  var _useState = useState(showTime),
      elapsed = _useState[0],
  ...
  return React.createElement(Alert, {
    ...
    variant: variant,
    title: title,
    actionClose: React.createElement(AlertActionCloseButton, {
      title: title,
...
/*
    ...
    limitations under the License.
*/
var TreeFocusContext = /*#__PURE__*/createContext(null);
var TreeFocusProvider = function TreeFocusProvider(_ref) {
  ...
  return React.createElement(TreeFocusContext.Provider, {
    value: {
...
var useTreeFocus = function useTreeFocus(_ref2) {
  ...
  var context = useContext(TreeFocusContext);
...
var Tree = function Tree(_ref) {
  ...
  return React.createElement(TreeFocusProvider, null, React.createElement("div", {
    ref: ref
...
/*
    ...
    limitations under the License.
*/
var TreeGroup = /*#__PURE__*/forwardRef(function TreeGroup(_ref, ref) {
  var id = _ref.id,
      expanded = _ref.expanded,
      level = _ref.level,
  ...
  var toggleExpandNoPropagation = useCallback(function (event) {
    event.stopPropagation();
    toggleExpand();
  }, [toggleExpand]);
  ...
  return React.createElement("div", Object.assign({
    ...
    isHidden: !isExpanded,
    className: css(styles$q.content, !isExpanded && styles$q.hiddenContent),
    role: "group"
  }, children({
    expanded: isExpanded,
    focused: focused || false
...
var TreeItem = /*#__PURE__*/forwardRef(function TreeItem(_ref, ref) {
...
function useInputTextSelectDialog(_ref) {
  ...
  var _useState3 = useState(textValue1.current),
      value1 = _useState3[0],
      setValue1 = _useState3[1];

  var _useState4 = useState(textValue2.current),
      value2 = _useState4[0],
  ...
  var modal = createPortal(React.createElement(Modal, {
    variant: "small",
    title: title,
    isOpen: isOpen,
    onClose: closeModal,
  ...
  }, textLabel1 && React.createElement(React.Fragment, null, React.createElement(TextInput, {
    ...
    autoFocus: true
  })), textLabel2.length > 0 && React.createElement(React.Fragment, null, React.createElement(TextInput, {
    className: styles$r.iGroupTextLabel,
    value: value2,
    type: "text",
  ...
  })), selectLabel && React.createElement(React.Fragment, null, React.createElement(Label, {
    className: styles$r.iSelectLabel
  }, selectLabel), React.createElement(FormSelect, {
...
function useLatestValue() {
  ...
  var previousElement = useRef();
  var handleSetElement = useCallback(function (newElement) {
...
var ContextToolbar = function ContextToolbar(_ref) {
  ...
  return React.createElement(Toolbar, {
    id: "data-toolbar",
    className: css(styles$s.toolbar),
    role: 'complementary'
  }, React.createElement(ToolbarContent, {
    className: css(styles$s.toolbarContent)
  }, children));
};
...
/*
    ...
    limitations under the License.
*/
var MainLayout = /*#__PURE__*/memo(function MainLayout(_ref) {
  ...
  return loading ? React.createElement(Loading, null) : React.createElement(Stack, {
    className: styles$v.view
  }, contextToolbar && React.createElement(StackItem, {
    isFilled: false
  }, contextToolbar), expressionToolbar && React.createElement(StackItem, {
    isFilled: false
  }, expressionToolbar), React.createElement(StackItem, {
    isFilled: true,
...
var MappingDetailsSidebar = function MappingDetailsSidebar(_ref) {
  ...
  return React.createElement(Stack, {
  ...
  }, React.createElement(StackItem, null, React.createElement(ColumnHeader, {
    ...
    actions: [React.createElement(Button, {
      ...
      key: 'close'
    }, React.createElement(CloseIcon, null)), React.createElement(Tooltip, {
      key: 'edit-enum',
      position: 'auto',
      enableFlip: true,
      content: React.createElement("div", null, "Edit the enumeration mappings"),
      entryDelay: 750,
      exitDelay: 100
    }, React.createElement(Button, {
      variant: 'plain',
      ...
      position: 'auto',
      enableFlip: true,
      content: React.createElement("div", null, "Remove the current mapping"),
      entryDelay: 750,
      exitDelay: 100
    }, React.createElement(Button, {
      variant: 'plain',
...
var CaptureDocumentIDAction = function CaptureDocumentIDAction(_ref) {
  ...
  return React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Capture Document ID to clipboard")
  }, React.createElement(Button, {
    variant: "plain",
    onClick: onClick,
...
var ChangeDocumentNameAction = function ChangeDocumentNameAction(_ref) {
  ...
  return React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Change document name")
  }, React.createElement(Button, {
    variant: "plain",
    onClick: onClick,
...
var DeleteDocumentAction = function DeleteDocumentAction(_ref) {
  ...
  return React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Remove instance or schema file")
  }, React.createElement(Button, {
    variant: "plain",
    onClick: onClick,
...
var DeleteMappingAction = function DeleteMappingAction(_ref) {
  ...
  return React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Remove the mapping")
  }, React.createElement(Button, {
    variant: "plain",
    onClick: onClick,
...
var EditCSVParamsAction = function EditCSVParamsAction(_ref) {
  ...
  return React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Modify CSV Parameters")
  }, React.createElement(Button, {
    variant: "plain",
    onClick: onClick,
...
var EditMappingAction = function EditMappingAction(_ref) {
  ...
  return React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Modify the mapping")
  }, React.createElement(Button, {
    variant: "plain",
    onClick: onClick,
    "aria-label": "Modify the mapping",
    "data-testid": "modify-the-mapping-" + id + "-button"
  }, React.createElement(ExchangeAltIcon, null)));
};
...
var EnableJavaClassAction = function EnableJavaClassAction(_ref) {
  ...
  return React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Load a Java document based on classes from your previously imported Java archive.")
  ...
  }, props), React.createElement(AddCircleOIcon, null)));
};
...
var ImportAction = function ImportAction(_ref) {
  ...
  var previouslyUploadedFiles = useRef(null);
  useEffect(function () {
  ...
  return React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Import instance or schema file")
  }, React.createElement(Button, {
    variant: "plain",
    "aria-label": "Import instance or schema file",
    "data-testid": "import-instance-or-schema-file-" + id + "-button",
    onClick: onClick
  }, React.createElement(ImportIcon, null), React.createElement(HiddenFileInput, {
...
var TreeGroupAndNodeRefsAndDnD = function TreeGroupAndNodeRefsAndDnD(_ref) {
  ...
  return React.createElement(FieldDropTarget, {
    ...
    return React.createElement(DelayedBoolean, {
      value: isOver
    }, function (isOver) {
      return React.createElement(NodeRef, {
        id: fieldId,
        ...
        renderLabel: function renderLabel(_ref3) {
          ...
          return React.createElement(DocumentGroup, {
            name: group.name,
            type: group.type,
            showType: showTypes,
            icon: group.isCollection ? React.createElement(LayerGroupIcon, null) : undefined,
  ...
  return React.createElement(FieldDropTarget, {
    ...
    return React.createElement(DraggableField, {
      ...
      return React.createElement(NodeRef, {
        ...
        return React.createElement(React.Fragment, null, React.createElement(DocumentField, {
          name: field.name,
          icon: isDroppable || isTarget ? React.createElement(Button, {
            variant: isTarget ? 'link' : 'plain',
            tabIndex: -1,
            "aria-label": 'Drop target',
            isDisabled: !isDroppable,
            "data-testid": "is-droppable-" + field.name + "-button"
          }, React.createElement(BullseyeIcon, {
            size: "sm"
          })) : React.createElement(Button, {
            variant: 'plain',
            tabIndex: -1,
          ...
          statusIcons: [field.isConnected ? React.createElement(Tooltip, {
            key: "connected",
            position: 'auto',
            enableFlip: true,
            entryDelay: 750,
            exitDelay: 100,
            content: React.createElement("div", null, "This field is connected")
          }, React.createElement(CircleIcon, {
            label: 'This field is connected',
            size: "sm",
            tabIndex: -1
          })) : null, field.isCollection ? React.createElement(Tooltip, {
            key: 'collection',
            position: 'auto',
            enableFlip: true,
            entryDelay: 750,
            exitDelay: 100,
            content: React.createElement("div", null, "This field is a collection")
          }, React.createElement(LayerGroupIcon, {
            label: 'This field is a collection',
            size: "sm",
            tabIndex: -1
          })) : null, field.hasTransformations ? React.createElement(Tooltip, {
            key: 'transformations',
            position: 'auto',
            enableFlip: true,
            entryDelay: 750,
            exitDelay: 100,
            content: React.createElement("div", null, "This field has transformations")
          }, React.createElement(BoltIcon, {
            label: 'This field has transformations',
            size: "sm",
            tabIndex: -1
          })) : null, field.isAttribute ? React.createElement(Tooltip, {
            key: 'attribute',
            position: 'auto',
            enableFlip: true,
            entryDelay: 750,
            exitDelay: 100,
            content: React.createElement("div", null, "This field is an attribute")
...
var TraverseFields = function TraverseFields(_ref) {
  ...
  return isVisible ? React.createElement(React.Fragment, null, fields.map(function (field, idx) {
    return React.createElement(FieldOrGroup, Object.assign({
      key: idx,
      field: field,
      idPrefix: idPrefix,
...
var FieldOrGroup = function FieldOrGroup(_ref2) {
  ...
  if (maybeGroup.fields) {
    return React.createElement(TreeGroupAndNodeRefsAndDnD, Object.assign({
      fieldId: fieldId,
      group: maybeGroup,
      level: level
    }, props), function (_ref3) {
      var expanded = _ref3.expanded;
      return React.createElement(TraverseFields, Object.assign({}, props
  ...
  return React.createElement(TreeItemWithFieldAndNodeRefsAndDnD, Object.assign({
    fieldId: fieldId,
    field: maybeField,
    level: level
  }, props));
...
function commonActions(_ref) {
  ...
  return [React.createElement(Tooltip, {
    ...
    position: 'top',
    enableFlip: true,
    content: React.createElement("div", null, "Show mapping details")
  }, connectedMappings.length > 1 ? React.createElement(AutoDropdown, {
    toggle: function toggle(_ref2) {
      var toggleOpen = _ref2.toggleOpen;
      return React.createElement(DropdownToggle, {
    ...
    dropdownItems: connectedMappings.map(function (m) {
      return React.createElement(DropdownItem, {
        ...
        headingLevel: "h2",
        size: "lg"
      }, m.name), React.createElement(Split, {
        hasGutter: true
      }, React.createElement(SplitItem, null, React.createElement(Title, {
        headingLevel: "h2",
        size: "md"
  ...
  }, React.createElement(ExchangeAltIcon, null))), canRemoveFromSelectedMapping ? React.createElement(Tooltip, {
    key: 'quick-remove',
    position: 'top',
    enableFlip: true,
    content: React.createElement("div", null, "Disconnect from the selected mapping")
  }, React.createElement(Button, {
    variant: "plain",
    ...
    tabIndex: 0
  }, React.createElement(UnlinkIcon, null))) : React.createElement(Tooltip, {
    key: 'quick-add',
    position: 'top',
    enableFlip: true,
    content: React.createElement("div", null, "Connect to the selected mapping")
  }, React.createElement(Button, {
    variant: "plain",
  ...
  }, React.createElement(LinkIcon, null))), React.createElement(Tooltip, {
    key: 'add',
    position: 'top',
    enableFlip: true,
    content: React.createElement("div", null, "Create new mapping")
  }, React.createElement(Button, {
    variant: "plain",
  ...
  }, React.createElement(ProjectDiagramIcon, null)))];
}
...
var ConstantsTree = function ConstantsTree(_ref) {
  var fields = _ref.fields,
      ...
      renderPreview = _ref.renderPreview;
  return React.createElement(Tree, null, React.createElement(TraverseFields, {
    fields: fields,
    showTypes: false,
    ...
    renderActions: function renderActions(field) {
      return [].concat(commonActions({
        ...
        position: 'top',
        enableFlip: true,
        content: React.createElement("div", null, "Edit constant"),
        entryDelay: 750,
        exitDelay: 100
      }, React.createElement(Button, {
        variant: "plain",
        ...
        position: 'top',
        enableFlip: true,
        content: React.createElement("div", null, "Remove constant"),
        entryDelay: 750,
        exitDelay: 100
      }, React.createElement(Button, {
        variant: "plain",
  ...
  return React.createElement(Tree, null, React.createElement(TraverseFields, {
    ...
    renderActions: function renderActions(field) {
      return [].concat(commonActions({
        ...
        position: 'top',
        enableFlip: true,
        content: React.createElement("div", null, "Edit property"),
        entryDelay: 750,
        exitDelay: 100
      }, React.createElement(Button, {
        variant: "plain",
        ...
        position: 'top',
        enableFlip: true,
        content: React.createElement("div", null, "Remove property"),
        entryDelay: 750,
        exitDelay: 100
      }, React.createElement(Button, {
        variant: "plain",
...
var SourcesColumn = function SourcesColumn(_ref) {
  var acceptDropType = _ref.acceptDropType,
      ...
      showTypes = _ref.showTypes;
  var renderPreview = useCallback(function (field) {
    return shouldShowMappingPreviewForField(field) && React.createElement(DocumentFieldPreview, {
      id: field.id,
      value: field.value,
      onChange: function onChange(value) {
  ...
  return React.createElement(React.Fragment, null, React.createElement(SearchableColumnHeader, {
    title: 'Source',
    onSearch: onSearch,
    actions: [onImportDocument && React.createElement(ImportAction, {
      id: "Source",
      onImport: onImportDocument,
      key: 'import'
    }), _onCustomClassSearch && React.createElement(EnableJavaClassAction, {
  ...
  }, React.createElement(ColumnBody, null, React.createElement(NodeRef, {
    id: SOURCES_WIDTH_BOUNDARY_ID
  }, React.createElement("div", null, React.createElement(NodeRef, {
    ...
    actions: [React.createElement(Tooltip, {
      position: 'top',
      enableFlip: true,
      content: React.createElement("div", null, "Create a source property for use in mapping"),
      key: 'create-property',
      entryDelay: 750,
      exitDelay: 100
    }, React.createElement(Button, {
  ...
  }, sourceProperties ? React.createElement(PropertiesTree, {
    acceptDropType: acceptDropType,
    draggableType: draggableType,
    isSource: isSource,
    ...
    showTypes: showTypes,
    renderPreview: renderPreview
  }) : 'No source properties')), React.createElement(NodeRef, {
    ...
    actions: [React.createElement(Tooltip, {
      position: 'top',
      enableFlip: true,
      content: React.createElement("div", null, "Create a constant for use in mapping"),
      key: 'create-constant',
      entryDelay: 750,
      exitDelay: 100
    }, React.createElement(Button, {
    ...
    noPadding: !!constants
  }, constants ? React.createElement(ConstantsTree, {
    ...
    renderPreview: renderPreview
  }) : React.createElement("p", null, "No constants"))), sources.map(function (s) {
    ...
    return React.createElement(NodeRef, {
      ...
      title: s.name,
      startExpanded: true,
      footer: showTypes ? React.createElement(DocumentFooter, null, "Source document type: ", s.type) : undefined,
      actions: s.type === DocumentType.CSV ? [onCaptureDocumentID && React.createElement(CaptureDocumentIDAction, {
        id: documentId,
        onClick: function onClick() {
          return onCaptureDocumentID(s.id);
        },
        key: 'capture-document-id'
      }), onEditCSVParams && React.createElement(EditCSVParamsAction, {
        id: documentId,
        onClick: function onClick() {
          return onEditCSVParams(s.id, true);
        },
        key: 'on-edit-csv-params'
      }), onDeleteDocument && React.createElement(DeleteDocumentAction, {
        id: documentId,
        onClick: function onClick() {
          return onDeleteDocument(s.id);
        },
        key: 'delete-document'
      })] : [onCaptureDocumentID && React.createElement(CaptureDocumentIDAction, {
        id: documentId,
        onClick: function onClick() {
          return onCaptureDocumentID(s.id);
        },
        key: 'capture-document-id'
      }), onChangeDocumentName && React.createElement(ChangeDocumentNameAction, {
        id: documentId,
        onClick: function onClick() {
      ...
      noPadding: true
    }, React.createElement(Tree, null, React.createElement(TraverseFields, {
      fields: s.fields,
      showTypes: showTypes,
      boundaryId: SOURCES_HEIGHT_BOUNDARY_ID,
      overrideWidth: SOURCES_WIDTH_BOUNDARY_ID,
...
var TargetsColumn = function TargetsColumn(_ref) {
  ...
  var renderPreviewResult = useCallback(function (field) {
    return shouldShowMappingPreviewForField(field) && React.createElement(DocumentFieldPreviewResults, {
      id: field.id,
      value: field.value
    });
  ...
  return React.createElement(React.Fragment, null, React.createElement(SearchableColumnHeader, {
    ...
    actions: [React.createElement(Tooltip, {
      position: 'top',
      enableFlip: true,
      content: React.createElement("div", null, "Create a target property for use in mapping"),
  ...
  }, targetProperties ? React.createElement(PropertiesTree, {
    acceptDropType: acceptDropType,
    draggableType: draggableType,
    isSource: isSource,
    onEditProperty: onEditProperty,
    ...
    return React.createElement(NodeRef, {
      ...
      title: t.name,
      startExpanded: true,
      footer: showTypes ? React.createElement(DocumentFooter, null, "Target document type: ", t.type) : undefined,
      actions: t.type === DocumentType.CSV ? [onCaptureDocumentID && React.createElement(CaptureDocumentIDAction, {
        id: documentId,
        onClick: function onClick() {
...
var SourceTargetLinks = function SourceTargetLinks(_ref) {
  ...
  var sortedMappings = useMemo(function () {
  ...
  return React.createElement("svg", {
    ...
    return React.createElement(MappingLines, {
      mapping: m,
      onClick: function onClick() {
...
var MappingLines = function MappingLines(_ref2) {
  ...
  var mappingLines = mapping.sourceFields.reduce(function (lines, start) {
    var linesFromSource = mapping.targetFields.map(function (end) {
      return React.createElement(NodesArc, {
        key: "" + start.id + end.id,
...
var MappingDetailsView = function MappingDetailsView(_ref) {
  ...
  var mappingAction = multiplicity && React.createElement(MappingTransformation, {
  ...
  var renderMappingField = function renderMappingField(isSource, canShowIndex, f, index) {
    return f ? React.createElement(MappingField, {
      key: f.id,
      field: f,
      name: f.name,
      ...
      return React.createElement(MappingTransformation, {
    ...
    }))) : React.createElement(MappingField, {
  ...
  return React.createElement(MappingDetailsSidebar, {
    onClose: onClose,
    onDelete: onRemoveMapping,
    onEditEnum: onEditEnum,
    isEnumMapping: isEnumMapping
  }, errors.length > 0 && React.createElement(NotificationsGroup, {
    notifications: errors,
    title: 'Errors',
    onNotificationRead: onNotificationRead
  }), warnings.length > 0 && React.createElement(NotificationsGroup, {
    notifications: warnings,
    title: 'Warnings',
    onNotificationRead: onNotificationRead
  }), messages.length > 0 && React.createElement(NotificationsGroup, {
    ...
    className: 'pf-c-form',
    style: formInputGroupStyle
  }, mappingAction, React.createElement(MappingFields, {
    isSource: true,
    title: 'Sources',
    onCreateConstant: onCreateConstant,
    onCreateProperty: onCreateProperty,
    canAddToSelectedMapping: canAddToSelectedMapping
  }, sources.map(renderSourceMappingField), targets.length <= 1 && React.createElement(AddFieldTypeahead, {
  ...
  })), React.createElement(MappingFields, {
    isSource: false,
    title: 'Targets',
    onCreateConstant: onCreateConstant,
    onCreateProperty: onCreateProperty,
    canAddToSelectedMapping: canAddToSelectedMapping
  }, targets.map(renderTargetMappingField), sources.length <= 1 && React.createElement(AddFieldTypeahead, {
...
var MappingTableView = function MappingTableView(_ref) {
  ...
  var rows = mappings.length === 0 ? emptyContent : mappings.map(function (mapping) {
    var sources = mapping.sourceFields.map(function (source, index) {
      ...
      return React.createElement("div", {
        key: index
      }, name, shouldShowMappingPreview(mapping) && React.createElement(DocumentFieldPreview, {
        id: field.id,
        value: field.value,
        onChange: function onChange(value) {
          return onFieldPreviewChange(field, value);
    ...
    var targets = mapping.targetFields.map(function (target, index) {
      ...
      return React.createElement("div", {
        key: index
      }, name, shouldShowMappingPreview(mapping) && React.createElement(DocumentFieldPreviewResults, {
        id: field.id,
        value: field.value
      }));
    });
  ...
  return React.createElement(MainContent, null, React.createElement(Title, {
    size: "lg",
    headingLevel: 'h1',
    className: styles$y.title
  }, "Mappings"), React.createElement(Table, {
    "aria-label": "Mappings",
    cells: columns,
    rows: rows
  }, React.createElement(TableHeader, null), React.createElement(TableBody, {
...
var NamespaceTableView = function NamespaceTableView(_ref) {
  ...
  var namespaceTables = sources.reduce(function (xmlSources, source) {
    ...
    return [].concat(xmlSources, [React.createElement(Level, {
      key: "level"
    }, React.createElement(LevelItem, {
      key: "title"
    }, React.createElement(Title, {
      size: "lg",
      headingLevel: 'h1',
      className: styles$z.title
    }, "Namespaces for ", source.name)), React.createElement(LevelItem, {
      key: "actions"
    }, React.createElement(Actions, null, React.createElement(Tooltip, {
      position: 'top',
      enableFlip: true,
      content: React.createElement("div", null, "Create a namespace"),
    ...
    }, React.createElement(PlusIcon, null)))))), React.createElement(Table, {
      key: "table",
  ...
  return namespaceTables.length === 0 ? React.createElement(Bullseye, null, React.createElement(EmptyState, {
    variant: EmptyStateVariant.small
  }, React.createElement(EmptyStateIcon, {
    icon: TableIcon
  }), React.createElement(Title, {
    headingLevel: "h2",
    size: "lg"
  }, "No XML sources found"))) : React.createElement(MainContent, null, namespaceTables);
};
...
var SourceTargetView = function SourceTargetView(_ref) {
  ...
  return React.createElement(React.Fragment, null, React.createElement(NodeRefProvider, null, React.createElement(ColumnMapper, {
    role: 'main'
  }, React.createElement(Columns, null, React.createElement(Column, {
    "data-testid": 'column-source-area',
    totalColumns: 2
  }, React.createElement(SourcesColumn, Object.assign({
    sourceProperties: sourceProperties,
    constants: constants,
    sources: sources,
    showTypes: showTypes
  }, sourceEvents))), React.createElement(Column, {
    "data-testid": 'column-target-area',
    totalColumns: 2
  }, React.createElement(TargetsColumn, Object.assign({
    targetProperties: targetProperties,
    showMappingPreview: showMappingPreview,
    showTypes: showTypes,
    targets: targets
  }, targetEvents))))), React.createElement(SourceTargetLinks, {
    mappings: mappings,
...
var AtlasmapToolbarItem = function AtlasmapToolbarItem(_ref) {
  ...
  var dropdownItems = [showImportAtlasFileToolbarItem && React.createElement(ImportAtlasFileToolbarItem, {
    onFile: runAndClose(onImportAtlasFile),
    key: "import-catalog"
  }), showImportJarFileToolbarItem && React.createElement(ImportJarFileToolbarItem, {
    onFile: runAndClose(onImportJarFile),
    key: "import-java-archive"
  }), (showImportAtlasFileToolbarItem || showImportJarFileToolbarItem) && React.createElement(DropdownSeparator, {
    key: "import-separator"
  }), showExportAtlasFileToolbarItem && React.createElement(ExportAtlasFileToolbarItem, {
    onClick: runAndClose(onExportAtlasFile),
    key: 'export-catalog'
  }), showExportAtlasFileToolbarItem && React.createElement(DropdownSeparator, {
    key: "export-separator"
  }), showResetToolbarItem && React.createElement(ResetToolbarItem, {
    onClick: runAndClose(onResetAtlasmap),
    key: "reset-catalog"
  })].filter(function (f) {
    return f;
  });
  return React.createElement(ToolbarItem, null, React.createElement(Dropdown, {
    toggle: React.createElement(DropdownToggle, {
      id: "atlasmap-toggle",
      onToggle: onToggle,
...
var ImportAtlasFileToolbarItem = function ImportAtlasFileToolbarItem(_ref2) {
  ...
  return React.createElement(DropdownItem, {
    icon: React.createElement(ImportIcon, null),
    onClick: onClick,
    "data-testid": "import-mappings-button"
  }, "Import a catalog (.adm)", React.createElement(HiddenFileInput, {
    accept: ".adm",
    multiple: false
  }));
};
var ImportJarFileToolbarItem = function ImportJarFileToolbarItem(_ref3) {
  ...
  var _useFilePicker2 = useFilePicker({
    maxFileSize: 1
  }),
  ...
  }, "Import a Java archive (.jar)", React.createElement(HiddenFileInput, {
    accept: ".jar",
    multiple: false
  }));
};
...
var ToggleColumnMapperViewToolbarItem = function ToggleColumnMapperViewToolbarItem(_ref7) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Show column mapper")
  }, React.createElement(Button, {
    variant: 'plain',
    "aria-label": "Show column mapper",
    onClick: onClick,
    isDisabled: disabled,
    className: css(toggled && styles$A.toggled),
...
var ToggleMappingTableViewToolbarItem = function ToggleMappingTableViewToolbarItem(_ref8) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Show mapping table")
  }, React.createElement(Button, {
    variant: 'plain',
...
var ToggleNamespaceTableViewToolbarItem = function ToggleNamespaceTableViewToolbarItem(_ref9) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Show namespace table")
  }, React.createElement(Button, {
    variant: 'plain',
...
var ToggleMappingPreviewToolbarItem = function ToggleMappingPreviewToolbarItem(_ref11) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Show/hide mapping preview")
  }, React.createElement(Button, {
    variant: 'plain',
...
var ToggleTypesToolbarItem = function ToggleTypesToolbarItem(_ref12) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Show/hide types")
  }, React.createElement(Button, {
    variant: 'plain',
...
var ToggleMappedFieldsToolbarItem = function ToggleMappedFieldsToolbarItem(_ref13) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Show/hide mapped fields")
  }, React.createElement(Button, {
    variant: 'plain',
  ...
  }, React.createElement(MapMarkedIcon, null))));
};
var ToggleUnmappedFieldsToolbarItem = function ToggleUnmappedFieldsToolbarItem(_ref14) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Show/hide unmapped fields")
  }, React.createElement(Button, {
    variant: 'plain',
  ...
  }, React.createElement(MapIcon, null))));
};
var AddMappingToolbarItem = function AddMappingToolbarItem(_ref15) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "Add a new mapping")
  }, React.createElement(Button, {
    variant: 'plain',
...
var AboutToolbarItem = function AboutToolbarItem(_ref16) {
  ...
  return React.createElement(ToolbarItem, null, React.createElement(Tooltip, {
    position: 'auto',
    enableFlip: true,
    content: React.createElement("div", null, "About AtlasMap")
  }, React.createElement(Button, {
    variant: 'plain',
...
var AtlasmapContext = /*#__PURE__*/createContext(null);
var AtlasmapProvider = function AtlasmapProvider(_ref) {
  ...
  var _useReducer = useReducer(dataReducer, {}, initDataState),
      data = _useReducer[0],
      dispatchData = _useReducer[1];

  var _useReducer2 = useReducer(notificationsReducer, {}, initNotificationsState),
  ...
  useEffect(function onInitializationCb() {
    ...
    if (externalDocument) {
      externalDocument.inputDocuments.forEach(function (d) {
        var inputDoc = new DocumentInitializationModel();
      ...
      var outputDoc = new DocumentInitializationModel();
  ...
  var convertSourcesToFlatArray = useCallback(function convertSourcesToFlatArrayCb() {
  ...
  var convertTargetsToFlatArray = useCallback(function convertTargetsToFlatArrayCb() {
  ...
  var onSubUpdate = useCallback(function onSubUpdateCb(_caller) {
    onUpdates({
  ...
  useEffect(function subscriptionListener() {
    var debounceTimeWindow = data.pending ? 1000 : 50;
    var initializationObservable = initializationService.systemInitializedSource.pipe(debounceTime(debounceTimeWindow));
    var lineRefreshObservable = configModel.mappingService.lineRefreshSource.pipe(debounceTime(debounceTimeWindow));
    var mappingUpdatedSource = configModel.mappingService.mappingUpdatedSource.pipe(debounceTime(debounceTimeWindow));
    var mappingPreview = configModel.previewService.mappingPreviewOutput$.pipe(debounceTime(debounceTimeWindow));
  ...
  useEffect(function onMappingChangeListenerCb() {
    if (onMappingChange) {
  ...
  return React.createElement(AtlasmapContext.Provider, {
...
function useAtlasmap() {
  ...
  var context = useContext(AtlasmapContext);
  ...
  var searchSources = useCallback(function (term) {
    return configModel.documentService.filterDocumentFields(term, true);
  }, [configModel]);
  var searchTargets = useCallback(function (term) {
    return configModel.documentService.filterDocumentFields(term, false);
  }, [configModel]);
  var handleImportADMArchiveFile = useCallback(function (file) {
    importADMArchiveFile(file, configModel);
  }, [configModel]);
  var handleImportJarFile = useCallback(function (file) {
    importJarFile(file, configModel);
  }, [configModel]);
  var handleResetAtlasmap = useCallback(function () {
    onReset();
    resetAtlasmap();
  }, [onReset]);
  var onAddToMapping = useCallback(function (node) {
    var field = node.amField;
    addToCurrentMapping(field);
  }, []);
  var onRemoveFromMapping = useCallback(function (node) {
    var field = node.amField;
    removeFromCurrentMapping(field);
  }, []);
  var onCreateMapping = useCallback(function (source, target) {
  ...
  var mappingHasSourceCollection = useCallback(function () {
  ...
  var canAddToSelectedMapping = useCallback(function (isSource) {
    var selectedMapping = context.selectedMapping;

    if (!selectedMapping || selectedMapping.mapping.transition.mode === TransitionMode.ENUM && selectedMapping.sourceFields.length > 0 && selectedMapping.targetFields.length > 0) {
      return false;
    }
  ...
  var isFieldAddableToSelection = useCallback(function (documentType, field, dropTarget) {
    var selectedMapping = context.selectedMapping;
    var isSource = documentType === 'source';

    if (!field || !field.amField.isTerminal() || (dropTarget === null || dropTarget === void 0 ? void 0 : dropTarget.type) === 'UNSUPPORTED' || selectedMapping && selectedMapping.mapping.transition.mode === TransitionMode.ENUM && selectedMapping.sourceFields.length > 0 && selectedMapping.targetFields.length > 0) {
      return false;
    }
  ...
  var isFieldRemovableFromSelection = useCallback(function (documentType, field) {
...
function useContextToolbar(_ref) {
  ...
  var _useState = useState('ColumnMapper'),
  ...
  var contextToolbar = useMemo(function () {
    return React.createElement(ContextToolbar, null, (showImportAtlasFileToolbarItem || showImportJarFileToolbarItem || showExportAtlasFileToolbarItem || showResetToolbarItem) && React.createElement(ToolbarGroup, {
      variant: "button-group",
      spacer: {
        "default": 'spacerMd'
      }
    }, React.createElement(AtlasmapToolbarItem, {
    ...
    })), (showColumnMapperViewToolbarItem || showMappingTableViewToolbarItem || showNamespaceTableViewToolbarItem) && React.createElement(ToolbarGroup, {
      variant: "icon-button-group",
      spacer: {
        "default": 'spacerMd'
      }
    }, showColumnMapperViewToolbarItem && React.createElement(ToggleColumnMapperViewToolbarItem, {
      toggled: activeView === 'ColumnMapper',
      onClick: function onClick() {
        return setActiveView('ColumnMapper');
      }
    }), showMappingTableViewToolbarItem && React.createElement(ToggleMappingTableViewToolbarItem, {
      toggled: activeView === 'MappingTable',
      onClick: function onClick() {
        return setActiveView('MappingTable');
      }
    }), showNamespaceTableViewToolbarItem && React.createElement(ToggleNamespaceTableViewToolbarItem, {
      ...
      variant: "icon-button-group"
    }, showToggleMappingPreviewToolbarItem && (activeView === 'ColumnMapper' || activeView === 'MappingTable') && React.createElement(ToggleMappingPreviewToolbarItem, {
      toggled: showMappingPreview,
      onClick: toggleShowMappingPreview
    }), showToggleTypesToolbarItem && (activeView === 'ColumnMapper' || activeView === 'MappingTable') && React.createElement(ToggleTypesToolbarItem, {
      toggled: showTypes,
      onClick: toggleShowTypes
    }), showToggleMappedFieldsToolbarItem && activeView === 'ColumnMapper' && React.createElement(ToggleMappedFieldsToolbarItem, {
      toggled: showMappedFields,
      onClick: toggleShowMappedFields
    }), showToggleUnmappedFieldsToolbarItem && activeView === 'ColumnMapper' && React.createElement(ToggleUnmappedFieldsToolbarItem, {
      toggled: showUnmappedFields,
      onClick: toggleShowUnmappedFields
    }), showAddNewMappingToolbarItem && (activeView === 'ColumnMapper' || activeView === 'MappingTable') && React.createElement(AddMappingToolbarItem, {
      onClick: newMapping
    }), showAddNewMappingToolbarItem && (activeView === 'ColumnMapper' || activeView === 'MappingTable') && React.createElement(AboutToolbarItem, {
      onClick: onAbout
    })));
...
function useConfirmationDialog(title, content) {
  ...
  return [React.createElement(ConfirmationDialog, {
    ...
    onConfirm: onConfirm,
    isOpen: state
  }, React.createElement(TextContent, null, content)), openDialog];
}
...
function useConstantDialog(title) {
  ...
  var onConfirm = useCallback(function (constant) {
    if (onConstantCb) {
  ...
  var dialog = React.createElement(ConstantDialog, Object.assign({
    title: title,
    isOpen: state,
    valueTypeOptions: constantTypes.map(function (_ref) {
      var value = _ref[0],
          label = _ref[1];
      return {
        value: value,
  ...
  var onOpenConstantDialog = useCallback(function (callback, constantsDoc, constant) {
...
function useCustomClassDialog(title) {
  ...
  var dialog = React.createElement(CustomClassDialog, Object.assign({
    title: title,
    isOpen: state,
    customClassName: customClassNames ? customClassNames[0] : '',
    customClassNames: customClassNames,
    collectionTypeOptions: collectionTypes.map(function (_ref) {
      var value = _ref[0],
          label = _ref[1];
      return {
        value: value,
  ...
  var onOpenCustomClassDialog = useCallback(function (callback, constant) {
    // we use a closure to set the state here else React will think that callback
    // is the function to retrieve the state and will call it immediately.
...
function useDeleteConstantDialog() {
  ...
  var onDeleteConstant = useCallback(function (constName) {
...
function useDeleteDocumentDialog() {
  ...
  var onDeleteDocument = useCallback(function (id, isSource) {
...
function useCreateToast() {
  ...
  var toastElement = React.createElement(TimedToast, {
    variant: 'info',
  ...
  if (state) {
    ...
    toastElement = React.createElement("span", null);
  }
...
function useCaptureDocumentIDToast() {
  ...
  var onCaptureDocumentID = useCallback(function (documentId) {
    docId = documentId;
...
function useChangeDocumentNameDialog() {
  ...
  var onConfirm = useCallback(function (documentNameInfo) {
    if (onDocumentNameCb) {
      onDocumentNameCb(documentNameInfo);
      toggleOff();
    }
  }, [onDocumentNameCb, toggleOff]);
  var dialog = React.createElement(ChangeDocumentNameDialog, Object.assign({
    isOpen: state,
    onCancel: toggleOff,
    onConfirm: onConfirm
  }, initialDocumentName || {}));
  var onOpenChangeDocumentNameDialog = useCallback(function (callback, documentNameInfo) {
...
function useDeleteMappingDialog() {
  ...
  var onDeleteMapping = useCallback(function (mapping) {
...
function useDeletePropertyDialog() {
  ...
  var onDeleteProperty = useCallback(function (propName, propScope, isSource) {
...
function useImportADMArchiveDialog() {
  ...
  var onImportADMArchive = useCallback(function (file) {
...
function useExportADMArchiveDialog() {
  ...
  var onExportADMArchiveFile = useCallback(function (filename) {
    exportADMArchiveFile(filename);
    toggleOff();
  }, [exportADMArchiveFile, toggleOff]);
  var dialog = React.createElement(ExportCatalogDialog, {
    isOpen: state,
    onCancel: toggleOff,
...
function useParametersDialog(title) {
  ...
  var onConfirm = useCallback(function (parameters) {
    if (onParametersCb) {
      onParametersCb(parameters);
      toggleOff();
    }
  }, [onParametersCb, toggleOff]);
  var dialog = React.createElement(ParametersDialog, {
    title: title,
    isOpen: state,
    onCancel: toggleOff,
    onConfirm: onConfirm,
  ...
  var onOpenParametersDialog = useCallback(function (callback, parameters) {
...
function useSpecifyInstanceSchemaDialog(defaultSchema) {
  ...
  }, React.createElement(React.Fragment, null, React.createElement(Radio, {
...
function useImportDocumentDialog() {
  ...
  var importFile = useCallback(function (selectedFile, isSource) {
  ...
  var onImportDocument = useCallback(function (selectedFile, isSource) {
  ...
  return [React.createElement(React.Fragment, null, importDialog, parametersDialog, specifyInstanceSchemaDialog), onImportDocument];
}
...
function useNamespaceDialog(title) {
  ...
  var onConfirm = useCallback(function (namespace) {
    if (onNamespaceCb) {
      onNamespaceCb(namespace);
      toggleOff();
    }
  }, [onNamespaceCb, toggleOff]);
  var dialog = React.createElement(NamespaceDialog, Object.assign({
    title: title,
    isOpen: state,
    onCancel: toggleOff,
    onConfirm: onConfirm
  }, initialNamespace || {}));
  var onOpenNamespaceDialog = useCallback(function (callback, namespace) {
    // we use a closure to set the state here else React will think that callback
    // is the function to retrieve the state and will call it immediately.
...
function usePropertyDialog(title, scopeOptions) {
  ...
  var onConfirm = useCallback(function (property) {
    if (onPropertyCb) {
  ...
  var dialog = React.createElement(PropertyDialog, Object.assign({
    title: title,
    valueTypeOptions: propertyTypes.map(function (_ref) {
      var value = _ref[0],
          label = _ref[1];
      return {
        value: value,
  ...
  var onOpenPropertyDialog = useCallback(function (callback, properties, property) {
    // we use a closure to set the state here else React will think that callback
    // is the function to retrieve the state and will call it immediately.
...
function useRemoveMappedFieldDialog() {
  ...
  var onRemoveMappedField = useCallback(function (removeMappedField) {
...
function useResetAtlasmapDialog() {
  ...
  var onResetAtlasmap = useCallback(function () {
...
function useAboutDialog() {
  ...
  var dialog = React.createElement(AboutDialog, {
  ...
  var onAboutDialog = useCallback(function () {
...
function useToggleExpressionModeDialog() {
  ...
  var onToggleExpressionMode = useCallback(function () {
...
function useEditMappingEnumerationDialog() {
  ...
  var dialog = React.createElement(ConfirmationDialog, {
    ...
    return React.createElement("div", {
      key: idx
    }, React.createElement(LookupTableDialog, {
      enumerationValue: value,
      sourceKey: idx,
      isOpen: state
    }));
  }));
  var onOpenDialog = useCallback(function () {
    getEnumValues();
    toggleOn();
  }, [toggleOn]);
...
function useAtlasmapDialogs(_ref) {
  ...
  var onCreateConstant = useCallback(function (constants, addToActiveMapping) {
  ...
  var onEditConstant = useCallback(function (constant, constants) {
  ...
  var _useState = useState(getPropertyScopeOptions(true)),
  ...
  var onCreateProperty = useCallback(function (isSource, properties, addToActiveMapping) {
  ...
  var onEditProperty = useCallback(function (property, isSource, properties) {
    setScopeOptions(getPropertyScopeOptions(isSource));
  ...
  var onChangeDocumentName = useCallback(function (docNameInfo) {
  ...
  var onCreateNamespace = useCallback(function (docName) {
  ...
  var onEditNamespace = useCallback(function (docName, namespace) {
  ...
  var onDeleteSelectedMapping = useCallback(function () {
    if (selectedMapping) {
  ...
  var onEnableCustomClass = useCallback(function (isSource) {
  ...
  function initialCSVParams(docId, isSource) {
    ...
    var initialCSVParameters = getCsvParameterOptions();
  ...
  var onEditCSVParams = useCallback(function (docId, isSource) {
...
function useSidebar(_ref) {
  ...
  return useCallback(function () {
    if (selectedMapping) {
      ...
      return React.createElement(MappingDetailsView, {
    ...
    return React.createElement(React.Fragment, null, "TODO: error");
...
var Atlasmap = function Atlasmap(_ref) {
  ...
  var shouldShowMappingPreviewForField = useCallback(function (field) {
  ...
  var shouldShowMappingPreview = useCallback(function (mapping) {
    return showMappingPreview && !!selectedMapping && mapping.id === selectedMapping.id;
  }, [selectedMapping, showMappingPreview]);
  var expressionToolbar = React.createElement(ExpressionToolbar, null, React.createElement(ConditionalExpressionInput, {
  ...
  var sourceEvents = useMemo(function () {
    return {
      isSource: true,
  ...
  var targetEvents = useMemo(function () {
    return {
      isSource: false,
  ...
  var currentView = useMemo(function () {
    switch (activeView) {
      case 'ColumnMapper':
        return React.createElement(SourceTargetView, {
          sourceProperties: sourceProperties,
          targetProperties: targetProperties,
          constants: constants,
          sources: sources,
      ...
      case 'MappingTable':
        return React.createElement(MappingTableView, {
          mappings: mappings,
          onSelectMapping: selectMapping,
      ...
      case 'NamespaceTable':
        return React.createElement(NamespaceTableView, {
          sources: sources,
  ...
  return React.createElement(FieldsDndProvider, null, React.createElement(CanvasProvider, null, React.createElement(MainLayout, {
    loading: pending,
    contextToolbar: contextToolbar,
    expressionToolbar: activeView !== 'NamespaceTable' && expressionToolbar,
    showSidebar: !!selectedMapping,
    renderSidebar: renderSidebar
  }, currentView), React.createElement(FieldDragLayer, null), React.createElement(AlertGroup, {
    isToast: true
    ...
    return React.createElement(TimedToast, {
      variant: variant,
      title: title,
      key: id,
      onClose: function onClose() {
...
export { AboutDialog, Actions, AddFieldTypeahead, Arc, ArcEdge, Atlasmap, AtlasmapProvider, AutoDropdown, Canvas, CanvasProvider, CanvasTransforms, CaptureDocumentIDAction, ChangeDocumentNameAction, ChangeDocumentNameDialog, Column, ColumnBody, ColumnHeader, ColumnMapper, Columns, ConditionalExpressionInput, ConfirmationDialog, ConstantDialog, ContextToolbar, CustomClassDialog, DelayedBoolean, DeleteDocumentAction, DeleteMappingAction, Document, DocumentActions, DocumentField, DocumentFieldPreview, DocumentFieldPreviewResults, DocumentFooter, DocumentGroup, DraggableField, DraggedField, EditCSVParamsAction, EditMappingAction, EnableJavaClassAction, ExportCatalogDialog, ExpressionContent, ExpressionNode, ExpressionToolbar, FieldDragLayer, FieldDropTarget, FieldNode, FieldsDndProvider, HTMLObject, ImportAction, Loading, LookupTableDialog, MAPPINGS_DOCUMENT_ID_PREFIX, MAPPINGS_DROP_TYPE, MAPPINGS_FIELD_ID_PREFIX, MAPPINGS_HEIGHT_BOUNDARY_ID, MAPPINGS_WIDTH_BOUNDARY_ID, MainContent, MainLayout, MappingDetailsSidebar, MappingDetailsView, MappingField, MappingFields, MappingTableView, MappingTransformation, NamespaceDialog, NamespaceTableView, NodeRef, NodeRefContext, NodeRefProvider, NodesArc, ParametersDialog, PropertyDialog, SOURCES_CONSTANTS_ID, SOURCES_DOCUMENT_ID_PREFIX, SOURCES_DRAGGABLE_TYPE, SOURCES_FIELD_ID_PREFIX, SOURCES_HEIGHT_BOUNDARY_ID, SOURCES_PROPERTIES_ID, SOURCES_WIDTH_BOUNDARY_ID, SearchableColumnHeader, Sidebar, SourceTargetLinks, SourceTargetView, SourcesColumn, TARGETS_DOCUMENT_ID_PREFIX, TARGETS_DRAGGABLE_TYPE, TARGETS_FIELD_ID_PREFIX, TARGETS_HEIGHT_BOUNDARY_ID, TARGETS_PROPERTIES_ID, TARGETS_WIDTH_BOUNDARY_ID, TargetsColumn, TextNode, TimedToast, TraverseFields, Tree, TreeFocusProvider, TreeGroup, TreeItem, TruncatedString, WithCanvas, addToCurrentMapping, changeDocumentName, commonActions, configModel, copyToClipboard, createConstant, createMapping, createNamespace, createProperty, deleteAtlasFile, deleteConstant, deleteNamespace, deleteProperty, deselectMapping, documentExists, editConstant, editNamespace, editProperty, enableCustomClass, errorInfoToNotification, errorLevelToVariant, errorMessageToString, errorTypeToString, executeFieldSearch, exportADMArchiveFile, fromDocumentDefinitionToFieldGroup, fromFieldToIFields, fromFieldToIFieldsGroup, fromFieldToIFieldsNode, fromMappedFieldToIMappingField, fromMappingDefinitionToIMappings, fromMappingModelToImapping, fromNamespaceModelToINamespace, getConstantType, getConstantTypeIndex, getCustomClassNameOptions, getDocCSVParams, getDocDef, getDocDefByName, getEnumerationValues, getExpression, getField, getFieldEnums, getMappingActions, getMappingExpression, getMultiplicityActionDelimiters, getMultiplicityActions, getPropertyScopeOptions, getPropertyType, getPropertyTypeIndex, getRuntimeVersion, getUIVersion, handleActionChange, handleIndexChange, handleMultiplicityArgumentChange, handleMultiplicityChange, handleNewTransformation, handleRemoveTransformation, handleTransformationArgumentChange, handleTransformationChange, importADMArchiveFile, importInstanceSchema, importJarFile, initializationService, initializeMappingExpression, isEnumerationMapping, mappingExpressionAddField, mappingExpressionClearText, mappingExpressionInit, mappingExpressionInsertText, mappingExpressionObservable, mappingExpressionRemoveField, newMapping, onFieldPreviewChange, removeDocumentRef, removeFromCurrentMapping, removeMappedFieldFromCurrentMapping, removeMapping, resetAtlasmap, selectMapping, setDocCSVParams, setSelectedEnumValue, toggleExpressionMode, toggleMappingPreview, toggleShowMappedFields, toggleShowUnmappedFields, trailerId, updateEnumerationValues, useAtlasmap, useAtlasmapDialogs, useBoundingCanvasRect, useCanvas, useDimensions, useFieldsDnd, useInputTextSelectDialog, useLatestValue, useNodeRect, useNodeRef, useToggle, useTreeFocus };
//# sourceMappingURL=atlasmap.esm.js.map
