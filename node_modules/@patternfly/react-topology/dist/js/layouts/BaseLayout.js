...
class BaseLayout {
    ...
    getLayoutNode(nodes, node) {
        ...
        if (!layoutNode) {
            layoutNode = this.getLayoutNode(nodes, element_utils_1.getClosestVisibleParent(node));
        }
        return layoutNode;
    }
    ...
    runLayout(initialRun, restart = true) {
        ...
        // create datum
        const leafNodes = element_utils_1.leafNodeElements(this.graph.getNodes()).filter(n => n.isVisible());
        const groups = element_utils_1.groupNodeElements(this.graph.getNodes()).filter(g => g.isVisible());
